...
export function Footer() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const [nextCategories, nextDistricts] = await Promise.all([
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-6">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mt-6 flex gap-3">
              {socialLinks.map((social) => (<a key={social.name} href={social.href} className="flex h-10 w-10 items-center justify-center rounded-full bg-muted text-muted-foreground transition-colors hover:bg-primary hover:text-primary-foreground" aria-label={social.name}>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3">
              {categories.slice(0, 5).map((category) => (<li key={category.id}>
                  <Link href={`/categories/${category.id}`} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                    {category.name}
                  </Link>
                </li>))}
            </ul>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3">
              {districts.slice(0, 5).map((district) => (<li key={district.id}>
                  <Link href={`/districts/${district.id}`} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                    {district.name}
                  </Link>
                </li>))}
            </ul>
