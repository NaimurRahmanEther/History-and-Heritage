...
import { getDynamicDistrictById, getDynamicProductsByDistrict, } from "@/lib/dynamic-data";
export default function DistrictPage({ params, }) {
    ...
    useEffect(() => {
        let active = true;
        const load = async () => {
            const foundDistrict = (await getDynamicDistrictById(id)) ?? null;
            if (!active)
                return;
            ...
            const products = await getDynamicProductsByDistrict(id);
            if (!active)
                return;
            setDistrictProducts(products);
            setHydrated(true);
        };
        load().catch(() => {
            if (!active)
      ...
      <main>
        <section className="relative overflow-hidden border-b border-border">
          <div className="absolute inset-0 -z-10">
            <Image src={district.image} alt={district.name} fill className="object-cover opacity-30" priority/>
        ...
        <section className="py-16 lg:py-24">
          <div className="mx-auto max-w-7xl px-4 lg:px-8">
            ...
            {districtProducts.length > 0 ? (<div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                {districtProducts.map((product) => (<ProductCard key={product.id} product={product}/>))}
              </div>) : (<div className="mt-10 rounded-lg border border-dashed border-border bg-muted/50 p-12 text-center">
