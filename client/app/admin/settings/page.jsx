...
import { getAdminSettings, getCurrentUser, isAdminUser, saveAdminSettings, } from "@/lib/admin-store";
...
export default function AdminSettingsPage() {
    ...
    const handleSave = async () => {
        ...
        await saveAdminSettings(normalized);
    ...
    const handleReset = async () => {
        ...
        await saveAdminSettings(resetState);
        toast({
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input type="email" value={settings.storeEmail} onChange={(event) => setSettings({ ...settings, storeEmail: event.target.value })}/>
            </div>
            <div className="space-y-2">
              <Label>Store Phone</Label>
              <Input value={settings.storePhone} onChange={(event) => setSettings({ ...settings, storePhone: event.target.value })}/>
            </div>
            <div className="space-y-2">
              <Label>Currency Symbol</Label>
              <Input value={settings.currencySymbol} onChange={(event) => setSettings({ ...settings, currencySymbol: event.target.value })}/>
            </div>
            <div className="space-y-2 md:col-span-2">
              <Label>Store Address</Label>
              <Input value={settings.storeAddress} onChange={(event) => setSettings({ ...settings, storeAddress: event.target.value })}/>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              ...
              <Input type="number" min={0} value={settings.shippingFee} onChange={(event) => setSettings({
            ...settings,
            ...
            <div className="space-y-2">
              ...
              <Input type="number" min={0} value={settings.freeShippingThreshold} onChange={(event) => setSettings({
            ...settings,
            ...
            <div className="space-y-2">
              ...
              <Input type="number" min={0} value={settings.lowStockThreshold} onChange={(event) => setSettings({
            ...settings,
